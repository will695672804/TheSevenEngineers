        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="px-6 py-4 text-left text-sm font-medium text-gray-900">Produit</th>
...
const ProductModal: React.FC<ProductModalProps> = ({ product, onSubmit, onClose }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    data.append('features', formData.features.split(',').map(f => f.trim()).filter(f => f).join(','));
          ...
          <div>
            ...
            <div className="flex items-center space-x-4">
              ...
              {product?.image && !selectedImage && (
                <span className="text-sm text-gray-500">Image actuelle: {product.image.split('/').pop()}</span>
              )}
            </div>
          </div>
